<h2 class="head-quantum"><?php echo $category['ForumCategory']['title'];?> </h2>

<div id="categoryStats">
    <?php if ($cupcake->user()) { ?>
    <div class="forumOptions fr">
        <?php if ($cupcake->hasAccess('mod', $category['ForumCategory']['id'])) {
            echo $html->link(__d('forum', 'Moderasi', true), array('controller' => 'categories', 'action' => 'moderate', $category['ForumCategory']['id']));
        } ?>
        <?php if ($category['ForumCategory']['status'] == 0) {
            if ($cupcake->hasAccess($category['ForumCategory']['accessPost'])) {
                echo $html->link(__d('forum', 'Buat Topik', true), array('controller' => 'topics', 'action' => 'add', $category['ForumCategory']['id']));
            } ?>
            <?php if ($cupcake->hasAccess($category['ForumCategory']['accessPoll'])) {
                echo $html->link(__d('forum', 'Buat Polling', true), array('controller' => 'topics', 'action' => 'add', $category['ForumCategory']['id'], 'poll'));
            }
        } else {
    ...
    if (!empty($category['Moderator'])) {
        foreach ($category['Moderator'] as $mod) {
            $moderators[] = $html->link($mod['User']['nama'], array('controller' => 'users', 'action' => 'profile', $mod['User']['id'])); 
        }   
    } ?>
    ...
    <tr id="category_<?php echo $subCat['id']; ?>"<?php if ($counter % 2) echo ' class="altRow"'; ?>>
        ...
        <td>
            ...
            if (!empty($subCat['LastTopic'])) {
                ...
                <em><?php echo $time->relativeTime($lastTime, array('userOffset' => $cupcake->timezone())); ?></em> <span class="gray"><?php __d('forum', 'by'); ?> <?php echo $html->link($subCat['LastUser']['nama'], array('controller' => 'users', 'action' => 'profile', $subCat['lastUser_id'])); ?></span>
            <?php } else {
...
<div id="topicWrap">
	...
	<table cellspacing="0" class="table">
    <tr>
        <th colspan="2"><?php echo $paginator->sort(__d('forum', 'Topik', true), 'Topic.title'); ?></th>
        <th><?php echo $paginator->sort(__d('forum', 'Penulis', true), 'User.nama'); ?></th>
        <th><?php echo $paginator->sort(__d('forum', 'Tanggal', true), 'Topic.created'); ?></th>
        <th><?php echo $paginator->sort(__d('forum', 'Post', true), 'Topic.post_count'); ?></th>
        <th><?php echo $paginator->sort(__d('forum', 'Dilihat', true), 'Topic.view_count'); ?></th>
        <th><?php echo $paginator->sort(__d('forum', 'Aktifitas', true), 'LastPost.created'); ?></th>
    </tr>
	...
	<?php foreach ($stickies as $topic) {
   	...
   	<tr<?php if ($counter % 2) echo ' class="altRow"'; ?>>
        ...
        <td class="ac"><?php echo $html->link($topic['User']['nama'], array('controller' => 'users', 'action' => 'profile', $topic['User']['id'])); ?></td>
        ...
        <td>
            ...
            if (!empty($topic['LastPost'])) {
                ...
                <span class="gray"><?php __d('forum', 'by'); ?> <?php echo $html->link($topic['LastUser']['nama'], array('controller' => 'users', 'action' => 'profile', $topic['Topic']['lastUser_id'])); ?></span>
