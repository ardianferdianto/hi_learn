<h2 class="head-quantum"><?php echo $topic['Topic']['title'];?> </h2>
<?php // Crumbs
...
<div id="postWrap">
			...
			if ($cupcake->user()) {
				...
				if ($cupcake->hasAccess('super', $topic['ForumCategory']['id']) || $cupcake->user('id') == $post['Post']['user_id']) {
					if ($topic['Topic']['firstPost_id'] == $post['Post']['id']) {
						...
						$links[] = $html->link(__d('forum', 'Hapus', true), array('controller' => 'topics', 'action' => 'delete', $topic['Topic']['id']), array('confirm' => __d('forum', 'Are you sure you want to delete?', true)));
						$links[] = $html->link(__d('forum', 'Laporan', true), array('controller' => 'topics', 'action' => 'report', $topic['Topic']['id']));
					} else {
						$links[] = $html->link(__d('forum', 'Edit', true), array('controller' => 'posts', 'action' => 'edit', $post['Post']['id']));
						$links[] = $html->link(__d('forum', 'Hapus', true), array('controller' => 'posts', 'action' => 'delete', $post['Post']['id']), array('confirm' => __d('forum', 'Are you sure you want to delete?', true)));
						$links[] = $html->link(__d('forum', 'Laporan', true), array('controller' => 'posts', 'action' => 'report', $post['Post']['id']));
					}
				}
				
				if ($cupcake->hasAccess($topic['ForumCategory']['accessReply'])) {
					$links[] = $html->link(__d('forum', 'Kutip', true), array('controller' => 'posts', 'action' => 'add', $topic['Topic']['id'], $post['Post']['id']));
				}
    ...
    <tr>
    	<td valign="top" style="width: 25%">
        	<h4><?php echo $html->link($post['User']['nama'], array('controller' => 'users', 'action' => 'profile', $post['User']['id'])); ?></h4>
        	...
        	<strong><?php __d('forum', 'Tanggal Bergabung'); ?>:</strong> <?php echo $time->niceShort($post['User']['created'], $cupcake->timezone()); ?><br />
            <strong><?php __d('forum', 'Total Topik'); ?>:</strong> <?php echo number_format($post['User']['totalTopics']); ?><br />
            <strong><?php __d('forum', 'Jumlah Tulisan'); ?>:</strong> <?php echo number_format($post['User']['totalPosts']); ?>
        </td>
        <td valign="top">
...
<div class="forumOptions">
	<?php if ($cupcake->hasAccess('mod', $topic['ForumCategory']['id'])) {
		echo $html->link(__d('forum', 'Moderasi', true), array('controller' => 'topics', 'action' => 'moderate', $topic['Topic']['id']));
	} ?>
	<?php if ($cupcake->hasAccess($topic['ForumCategory']['accessPost'])) {
		echo $html->link(__d('forum', 'Buat Topik', true), array('controller' => 'topics', 'action' => 'add', $topic['ForumCategory']['id']));
	} ?>
    <?php if ($cupcake->hasAccess($topic['ForumCategory']['accessPoll'])) {
		echo $html->link(__d('forum', 'Buat Polling', true), array('controller' => 'topics', 'action' => 'add', $topic['ForumCategory']['id'], 'poll'));
	} ?>
    <?php if ($cupcake->hasAccess($topic['ForumCategory']['accessReply'])) {
		if ($topic['Topic']['status'] == 0) {
			echo $html->link(__d('forum', 'Balas', true), array('controller' => 'posts', 'action' => 'add', $topic['Topic']['id']));
		} else {
			echo '<span>'. __d('forum', 'Closed', true) .'</span>';
		}
	} ?>
...
<div id="quickReply">
	<h3><?php __d('forum', 'Balas Cepat'); ?></h3>
    
    <?php echo $form->create('Post', array('url' => array('controller' => 'posts', 'action' => 'add', $topic['Topic']['id']))); ?>
    <table cellspacing="0" class="table">
    <tr>
    	<td style="width: 25%">
        	<strong><?php echo $form->label('content', __d('forum', 'Pesan', true) .':'); ?></strong><br /><br />
            
            <?php echo $html->link(__d('forum', 'Pembalasan Lengkap', true), array('controller' => 'posts', 'action' => 'add', $topic['Topic']['id'])); ?><br />
    ...
    <tr class="altRow">
    	<td colspan="2" class="ac">
        	<?php echo $form->submit(__d('forum', 'Tulis Balasan', true)); ?>
        </td>
   	</tr> 
    </table>
    <?php echo $form->end(); ?>
