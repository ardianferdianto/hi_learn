 ...
 */
	...
	var $appLicationName = 'ELEARNING INTEGRATED';
/**
 ...
 */
	...
	//default licoff
	var $licoffdefault ='1-4-3-1318566926-eccbc-ELEARN';
	
	//URL SITE
	var $urlDefault ='http://localhost/elearn/';
/**
 * beforeFilter
 *
 * @return void
 */
    function beforeFilter() {
		...
		$appLicationName = 'ELEARNING INTEGRATED';
		$this->set('appLicationName',$appLicationName);
		$urlSite = 'http://localhost/elearn/';
        ...
        if (!empty($this->data)) {
			...
			if ($this->Data->validates())
			{
				...
				elseif (!empty($this->data['Key']['keyfeedback']))
				{
					...
					$licValGetX = 'ion:'.$licoff.'znd:'.$zend_id;
					$licValGet = preg_replace('/\s+/', '', $licValGetX);
	...
	function runDbApp1(){
		if (isset($this->params['named']['run'])) {
            ...
            if(!$db->isConnected()) {
                ...
                $this->__executeSQLScript($db, CONFIGS.'sql'.DS.'elearn_sma.sql');
				//$this->__saveInifile();
